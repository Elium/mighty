  ...
  save(request: IHttpRequest): Observable<IHttpResponse> {
    ...
    if (index < 0) {
      return Observable.throw(new HttpResponse({
        error: new Error("There is no match for this row criteria"),
        status: 400
      }))
    } else {
